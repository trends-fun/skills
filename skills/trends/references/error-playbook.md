...
| `iao project get requires a non-empty hash` | Empty or whitespace hash input | Provide a valid project hash and retry | `trends-skill-tool iao project get <hash>` |
...
| `Image file does not exist: <path>` | Invalid local path for `--image-path` in `iao create` | Use an existing local image path, or remove `--image-path` to use the selected project cover image | `trends-skill-tool iao create --project-url <url> --name \"My Coin\" --symbol \"MYC\"` |
| `iao create requires a project cover image when --image-path is omitted` | Selected project has no usable cover image and no local `--image-path` was provided | Select a project with a cover image, or provide a local `--image-path`; do not suggest symbol auto-generation for IAO | `trends-skill-tool iao create --project-url <url> --name \"My Coin\" --symbol \"MYC\" --image-path ./logo.png` |
